...
class Seq2Seq(nn.Module):
    ...
    """
    def __init__(self, encoder,decoder,config,beam_size=None,max_length=None,sos_id=0,eos_id=2,padding_id=1):
    ...
    def forward(self, source_ids=None,target_ids=None):   
        source_mask = source_ids.ne(self.padding_id).int()
        outputs = self.encoder(source_ids, attention_mask=source_mask)
        encoder_output = outputs["last_hidden_state"].permute([1,0,2]).contiguous()
        if target_ids is not None:
            tgt_mask = target_ids.ne(self.padding_id)
            attn_mask=-1e4 *(1-self.bias[:target_ids.shape[1],:target_ids.shape[1]])
            tgt_embeddings = self.encoder.embeddings(target_ids).permute([1,0,2]).contiguous()
            out, attentions = self.decoder(tgt_embeddings,encoder_output,tgt_mask=attn_mask,memory_key_padding_mask=(1-source_mask).bool())
            ...
            #Predict 
            preds=[]
            zero=torch.cuda.LongTensor(1).fill_(self.padding_id)     
...
class Seq2Seq4unixcoder(nn.Module):
    """
        Build Seqence-to-Sequence.
    ...
    def __init__(self, encoder,decoder, config, beam_size=None, max_length=None, sos_id=None, eos_id=None):
        super(Seq2Seq4unixcoder, self).__init__()
        self.encoder = encoder
        self.decoder=decoder
        self.config=config
    ...
    def forward(self, source_ids, target_ids=None):   
        ...
        mask = self.bias[:,source_ids.size(-1):ids.size(-1),:ids.size(-1)].bool() # 1*128*256
        mask = mask & ids[:,None,:].ne(1) # 10*128*256

        decoder_out = self.decoder(target_ids,attention_mask=mask, past_key_values=encoder_output.past_key_values)
        out = decoder_out.last_hidden_state
        attentions = decoder_out.attentions
    ...
    def generate(self, source_ids):
        ...
        encoder_output = self.encoder(source_ids,attention_mask=mask,use_cache=True)        
        preds = []       
        zero = torch.cuda.LongTensor(1).fill_(1)   
